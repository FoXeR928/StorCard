...
async def get_users_api(response: Response, current_user=Depends(get_current_user)):
    result = await get_users_query(current_user)
...
async def registration_user_api(
    ...
    result = await registration_user_query(current_user, **data.model_dump())
...
async def delet_user_api(
    ...
    result = await delete_user_query(requester=current_user, login=login)
    response.status_code = result.get("code", 201)
    return result
