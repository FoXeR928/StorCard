...
async def update_card_name_api(
    response: Response, name:UpdateCardName, current_user: User = Depends(get_current_user)
):
...
async def update_card_about_api(
    response: Response, about:UpdateCardAbout, current_user: User = Depends(get_current_user)
):
    result = update_card_about_query(card_id=about.id,user=current_user,about=about.about)
    response.status_code = result["cod"]
    del result["cod"]
    return result
...
async def update_card_image_api(
    response: Response, id:int,file:UploadFile, current_user: User = Depends(get_current_user)
):
    result = update_card_image_query(card_id=id,user=current_user,image=file.file.read())
    response.status_code = result["cod"]
    del result["cod"]
    return result
